...
module.exports = {
    ...
    aliases: ["updatetag", "u-tag", "tag-u"],
    command_usage: "Takes 3 arguments in 2 staggered messages,\nthe action (0=remove, 1=add, 2=edit),\nthe selected tag\nand a new tag\n```\nb!update-tag <0/1/2> <tag name>\n<then enter new tag message>\n```",
    run: (client, message, args) => {
        //Stores action to be taken in action
        const action = args[0];
        //If no second argument is supplied or first argument isn't 0||1, return
        if (!args[1] || (action != 0 && action != 1 && action != 2)) {
        ...
        if (action == 0) {
            ...
            //Occurs if action is to add a tag
        } else if (action == 1) {
            ...
            return;
            //Occurs if action is to edit a tag
        } else {
            //If there is no object in tags.json for the argument, return
            if (!jsonData[tag]) {
                message.channel.send("THIS TAG DOESN'T EXIST!");
                return;
            }
